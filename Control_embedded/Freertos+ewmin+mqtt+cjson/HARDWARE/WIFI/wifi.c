...
#define DMA_Send_Len   512   //发送数据长度 
#define DMA_Rec_Len   512    //接收数据长度
...
//				printf("标记3");
		   ...
		   DMA1_CH6->CNDTR = 512; //修改DMA1数据传输量

				

			memset(receivebuff->buffer,0x00,sizeof(receivebuff->buffer));
			//传输消息队列
//					printf("标记4");
				printf("WIFI模组接收到数据\r\n");
				receivebuff->length=DATA_LEN;
//					printf("标记5");
				
				memcpy(receivebuff->buffer,DMA_Receive_Buf,512);
//				printf("mem标记");
